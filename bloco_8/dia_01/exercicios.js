...
const drawnResult = luckyNum => {
    const numberDrawn = Math.ceil(Math.random() * 5);
    if (luckyNum === numberDrawn) {
        return 'Parabéns você ganhou! =D';
    } 
    return 'Tente novamente. =(';
}
    

// console.log(drawnResult(4));
...
const score = (template, answers) => {
    ...
    for (let index = 0; index < template.length; index += 1) {
        if (answers[index] === template[index]) {
            counter += 1;
        } else if(answers[index] !== template[index] & answers[index] !== 'N.A') {
            counter -= 0.5;
        }
    }
    return `Sua nota é: ${counter}`;
}

const checkAnswers = (template, answers, callback) => {
    return callback(template, answers);
}

// console.log(checkAnswers(rightAnswers, studentAnswers, score));