...
// Exercício Bônus - Vamos adicionar compromissos ao seu calendário? Implemente uma função que, ao digitar um compromisso na caixa de texto "COMPROMISSOS", adiciona o item à lista "MEUS COMPROMISSOS" ao clicar no botão "ADICIONAR".
/*
...
function createCompromise() {
    const wrtCompromise = document.getElementById('task-input');
    const addCompromiseBtn = document.getElementById('btn-add');
    wrtCompromise.addEventListener('keydown', function(event) {
        let key = event.keyCode;
        if (key == 13 && wrtCompromise.value !== '') {
            const divTag = document.createElement('div');
            divTag.className = 'compromises';
            divTag.innerHTML = wrtCompromise.value;
            document.querySelector('body').appendChild(divTag);
        }
        if (key == 13 && wrtCompromise.value === '') {
            alert('Erro! Seu compromisso jumento. Me dê papai!');
        }
    })
    addCompromiseBtn.addEventListener('click', function() {
        if (wrtCompromise.value !== '') {
            const divTag = document.createElement('div');
            divTag.className = 'compromises';
            divTag.innerHTML = wrtCompromise.value;
            document.querySelector('body').appendChild(divTag);
